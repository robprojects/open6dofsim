...
Geo6Dof::Geo6Dof(QWidget *parent) : QDialog(parent) {
	m_config.radius_base = 0.747;
	m_config.radius_platform = 0.5623;
	m_config.mid_length = 0.85163;
	m_config.min_length = 0.70986;
	m_config.range = 0.2835;
	m_config.sep_angle = 0.316;
	m_config.sep_angle_platform = 0.178;

	init_geometry(&m_geo, m_config.radius_base, m_config.radius_platform, m_config.mid_length,
		m_config.min_length, m_config.range, m_config.sep_angle, m_config.sep_angle_platform);
...
#define GET_VALUE(X) m_config.X = ui.X->value();

void Geo6Dof::clickOK() {
	GET_VALUE(radius_base);
	GET_VALUE(radius_platform);
	GET_VALUE(mid_length);
	GET_VALUE(min_length);
	GET_VALUE(range);
	GET_VALUE(sep_angle);
	GET_VALUE(sep_angle_platform);
...
#define SET_VALUE(X) ui.X->setValue(m_config.X)

void Geo6Dof::openConfigDialog() {
	SET_VALUE(radius_base);
	SET_VALUE(radius_platform);
	SET_VALUE(mid_length);
	SET_VALUE(min_length);
	SET_VALUE(range);
	SET_VALUE(sep_angle);
	SET_VALUE(sep_angle_platform);
	show();
}

void Geo6Dof::getLengths(int* len) {
	int i;
	for (i = 0; i < 6; i++) {
		len[i] = (int)((1.0 - ((m_state.length[i] - m_geo.act_min) / m_geo.act_range)) * 65535.0);
...
void Geo6Dof::resetPos() {
	const int stop_height = 0;
	...
	m_pos.T[0] = 0.0;
	m_pos.T[1] = 0.0;
	m_pos.T[2] = m_geo.min_height + ((m_geo.mid_height - m_geo.min_height) / 500.0) * ((float)stop_height);

	inverse_kinematics(&m_geo, &m_pos, &m_state);
}

void Geo6Dof::setStoppedPos(int stop_height) {
	m_stopped_pos.psi = 0.0;
	m_stopped_pos.theta = 0.0;
	m_stopped_pos.phi = 0.0;
	m_stopped_pos.T[0] = 0.0;
	m_stopped_pos.T[1] = 0.0;
	m_stopped_pos.T[2] = m_geo.min_height + ((m_geo.mid_height - m_geo.min_height) / 500.0) * ((float)stop_height) - m_geo.mid_height;
	setPos(&m_stopped_pos);
}

void Geo6Dof::setPos(motion_pos* pos)
{
	m_pos = *pos;

	//qDebug("Pos T %f %f %f", m_pos.T[0], m_pos.T[1], m_pos.T[2]);

	m_pos.T[2] += m_geo.mid_height;
...
int Geo6Dof::init_geometry(struct motion_geo *geo, double radius_base, double radius_platform, double mid_length, double min_length, double range, double sep_angle, double sep_angle_platform) {
...
int Geo6Dof::inverse_kinematics(struct motion_geo *geo, struct motion_pos *pos, struct motion_state *st) {
