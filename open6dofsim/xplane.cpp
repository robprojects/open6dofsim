...
int XPlane::register_all()
{
	int i;
	for (i = 0; i < 15; i++) {
		//printf("%s %d\n", dataref_name[i], i);
		register_dataref(dataref_name[i], i, m_sample);
		//qDebug("register %s", dataref_name[i]);
	}
	return 0;
}

int XPlane::register_dataref(char* name, int nr, int freq) {
	char buf[BUFLEN];
	...
	m_s->writeDatagram(buf, sizeof(rref_in) + 5, QHostAddress(m_server), PORT);
...
XPlane::XPlane (QObject *parent) : QObject(parent) {
	// xplane udp
	m_server = strdup("127.0.0.1");
	m_sample = SAMPLE;
	m_paused = 1;

	qDebug("Xplane");

	//m_s->connectToHost(QHostAddress("127.0.0.1"), PORT, QIODevice::ReadWrite);
...
void XPlane::setActive() {
	m_s = new QUdpSocket(this);
	m_enabled = true;
	connect(m_s, SIGNAL(readyRead()), this, SLOT(readyRead()));
	m_timer->start(1000);
}

void XPlane::setInactive() {
	m_enabled = false;
	m_s->deleteLater();
	m_timer->stop();
}

int XPlane::ReadNext(double* Faa, double* Oaa) {
...
QString& XPlane::getStatus()
{
	static QString status_wait = QString("Waiting...");
	static QString status_recv = QString("Receiving...");
	static QString status_paused = QString("Receiving... (paused)");
	switch (m_state) {
		case RECEIVING: return status_recv;
...
XPlane::~XPlane()
{
	//if (m_s) m_s->deleteLater();
	//if (m_timer) m_timer->deleteLater();
}

void XPlane::handleTimeout()
{
	qDebug("Xplane Timeout");
	m_state = WAITING;
	register_all();
	m_timer->start(1000);
}
...
void XPlane::readyRead() {
	struct dref_struct_out* dref;
	int cnt = 0;
	if (!m_enabled) return;
	int recv_len = m_s->readDatagram(m_buf, BUFLEN);

	if (recv_len < 0) return;

	dref = (struct dref_struct_out*)(m_buf + 5);
	...
	m_state = m_paused ? PAUSED : RECEIVING;
}
